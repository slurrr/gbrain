...
- `.pi/extensions/gbrain.ts` registers the full native memory/page/timeline/graph bridge, while `.pi/alex-policy.json` now exposes only read/search/edit/write plus Alex's six interactive memory tools: recall, remember, entity, forget, get_page, and put_page. The bridge reapplies the allowlist after registration and before each turn; ambient context plumbing and skill discovery remain unchanged.
...
- Validate the new ambient path with a real Pi turn: volunteer pointers, boundary pack after compaction, periodic delta, and explicit memory/page/graph tool round-trips. Runtime inspection confirmed the narrowed 12-tool Pi allowlist; AgentMux is serving enough for that inspection, but gbrain model-backed operations remain untested end-to-end.
